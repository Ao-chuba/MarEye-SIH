...
export default function JetsonPage() {
  ...
  useEffect(() => {
    try {
      const savedResults = localStorage.getItem('jetson-processing-results')
      if (savedResults) {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/jetson/dummy", {
        method: "POST",
        body: formData,
      })
      ...
      if (result.success) {
        ...
        } else if (error.message.includes("Failed to load ONNX model")) {
          errorMessage = "ONNX model loading failed. Please try again."
  ...
  const processVideo = async () => {
    ...
    try {
      ...
      console.log("Sending request to /api/jetson/dummy")
      ...
      let response
      try {
        response = await fetch("/api/jetson/dummy", {
          method: "POST",
          body: formData,
      ...
      if (result.success) {
        ...
        setResults(prev => {
          ...
          try {
            localStorage.setItem('jetson-processing-results', JSON.stringify(newResults))
          } catch (error) {
